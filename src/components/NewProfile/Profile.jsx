...
const Profile = (props) => {
    ...
    useEffect(() => {
        setSubmit(true);
        if (props.uid === authStatus.token) {
            dispatch(fetchProfileData(localStorage.getItem("token"))).then(
                (response) => {
        ...
        setCurUser(false);
        dispatch(fetchOtherProfileData("62441d11dc5a73f2cfe0f172")).then((res) => {
            console.log(res); //setting the above obtained data into the otherProfileData variable using useState
            setSubmit(false);
        });
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);
    return submit ? <LoadingSpinner /> : <div></div>;
